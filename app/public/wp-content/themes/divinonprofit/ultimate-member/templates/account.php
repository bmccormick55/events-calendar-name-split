...
<div class="um <?php echo esc_attr($this->get_class($mode)); ?> um-<?php echo esc_attr($form_id); ?>">
    ...
    <?php
        ...
        // Get the keys of the subscriptions
        $subscriptions_keys = array_keys( $subscriptions );


        foreach ( $subscriptions_keys as $sub ) {

            // Get the subscription data
            $sub_data = new WC_Subscription( $sub );

            // Get the items in the subscription
            foreach ( $sub_data->get_items() as $item_id => $product_subscription ) {
                
                // Get the subscription name
                $product_name = $product_subscription->get_name();

                // Convert the subscription name to lowercase
                $subscription_name = strtolower( $product_name );
            }
        }
    ?>

    <?php 
    // Check if the user subscription has been set 
    if ( $subscription_name != '' ) { ?>
        <div class="member-notification">
            <?php
                if (str_contains($subscription_name, 'corresponding')) {
                    echo get_field( 'corresponding_members_notification', 'option' );
                } elseif ( str_contains($subscription_name, 'active') || str_contains($subscription_name, 'associate') ) {
                    echo get_field( 'active_and_associate_members_notification', 'option' );
                }
            ?>
        </div>
    <?php } ?>
            ...
            <div class="um-account-meta radius-<?php echo esc_attr(UM()->options()->get('profile_photocorner')); ?> uimob340-show uimob500-show">
                ...
                <div class="um-account-name">
                    ...
                    <div class="um-account-profile-link">
                        <a href="<?php echo esc_url(um_get_core_page('logout')); ?>" class="um-link">
